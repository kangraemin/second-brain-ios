...
### 작업 내용
- feat: 코사인 유사도 기반 벡터 검색 서비스 구현
- VectorSearchService에 코사인 유사도 계산 및 임계값 기반 검색 구현.
- 7개 테스트 케이스 추가 (동일/반대/직교 벡터, 임계값 필터, 정렬 등).
...
### 작업 내용
- feat: 키워드 + 시맨틱 하이브리드 검색 결과 병합 구현
- SearchClientLive에서 키워드 검색과 벡터 시맨틱 검색 결과를
- 가중 점수(키워드 0.6 + 시맨틱 0.4)로 병합하여 랭킹

### 변경 파일
- `Stash/Data/Clients/SearchClientLive.swift`
...
### 작업 내용
- feat: 설정 화면 구현 및 홈 네비게이션 연동
- SettingsFeature Reducer (앱 버전, 저장 콘텐츠 수, 전체 삭제),
- SettingsView UI (List 기반), HomeView 툴바에 설정 버튼 추가.

### 변경 파일
- `Stash.xcodeproj/project.pbxproj`
- `Stash/Features/Home/HomeFeature.swift`
- `Stash/Features/Home/HomeView.swift`
- `Stash/Features/Settings/SettingsFeature.swift`
- `Stash/Features/Settings/SettingsView.swift`
...
### 작업 내용
- feat: 빈 상태 UI 개선 및 에러 처리 Alert 추가
- EmptyStateView에 아이콘 추가 (tray, magnifyingglass).
- 콘텐츠 로드 실패 시 재시도 가능한 Alert 표시.
